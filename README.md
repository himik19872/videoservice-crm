...
### v1.4 — 26.07.2026
- ⚡ **Быстрые шаблоны в заявках** — 23 частые проблемы (доводчик, ЭМЗ, домофон, УКП, ключи...) одним кликом
- 🔍 **Переделан поиск клиентов** — автокомплит по улицам из базы, фильтры по району/УК, поиск по лицевому счёту
- 🏢 **Страница домов**: колонка УК/ТСЖ, поиск по адресу и УК с debounce
- 🏗️ **Генерация квартир** при создании УК — кнопка «Сгенерировать квартиры» в карточке компании и домов
- 🔑 **Импорт Beward**: исправлено сопоставление IP/кодов с домами (по улице+дому+корпусу вместо одного номера дома)
- 📤 **Экспорт Beward в Excel** — сводная таблица устройств + подъездов
- 🏛️ **DaData Party API** — поиск компании по ИНН при создании УК
- 🐛 **Исправления**: район для пригородов СПб (Пушкин, Ломоносов), NoneType в парсинге Beward, Daphne timeout для импорта ТСЖ

### v1.3 — 19.07.2026
- ☎️ **Asterisk PBX** — AMI/SSH интеграция: SIP, транки, маршруты, IVR, voicemail, генератор конфигов, дашборд
- 📍 **Места хранения** — ячейки с зонами/стеллажами/полками, штрихкоды, сканер, перемещение, пересчёт
- 📞 **Ростелеком АТС** — CDR журнал звонков, синхронизация, привязка к клиентам
- 🔄 **Битрикс24** — двусторонняя синхронизация клиентов и товаров через вебхук
- 📥 **Импорт v3**: автоопределение формата ЕРЦ, 7 форматов, двухшаговый импорт, общежития
- 📊 **ЕРЦ начисления** — агрегация по месяцам, виджеты собираемости, таблица с прогресс-барами

### v1.2 — 12.07.2026
- 📦 **Склад v2**: поставщики, приходные накладные, штрих-коды, расходные ордера
- 💬 **Диалоги в заявках** — чат сотрудников с историей
- 🧾 **Сметы**: создание, PDF, привязка к заявке
- 💰 **Финансы**: оплаты, касса, зарплаты мастеров
- 🏢 **14 ролей** с гибкими правами

### v1.1 — 05.07.2026
- 📱 Мобильное приложение (Expo APK): заявки, GPS, фото/видео, подпись, смены, офлайн
- 🤖 Max Бот: уведомления, webhook
- 🗺️ Yandex Maps, GPS-трекинг, смены мастеров

### v1.0 — 01.07.2026
- Базовая CRM: заявки, клиенты, мастера, оборудование
- 5 ролей, разграничение доступа, дашборд
- DaData-подстановка адресов
...
| Страниц веб-интерфейса | 20 |
| Экранов мобильного | 9 |
| Интеграций | 7 (Max, DaData, Traccar, Expo Push, Asterisk, Bitrix24, Ростелеком АТС) |
